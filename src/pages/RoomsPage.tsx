...
export default function RoomsPage() {
  ...
  const openAddModal = () => {
    setIsEditing(false);
    setFormData({ id: null, hotel_id: hotels[0]?.id || "", room_type: "", price_per_night: "", capacity_adults: "", available_count: "", image: "", image_file: null, description: "" });
    setIsModalOpen(true);
  };
  ...
  const openEditModal = (room: any) => {
    ...
    setFormData({ 
      ...
      image: room.image || "" || "", image_file: null,
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      if (isEditing) {
        ...
        await axiosClient.post(`/admin/rooms/${formData.id}`, submitData, { headers: { 'Content-Type': 'multipart/form-data' } });
      } else {
        await axiosClient.post(`/admin/rooms`, submitData, { headers: { 'Content-Type': 'multipart/form-data' } });
      }
      closeModal();
      fetchRooms();
    } catch (err) {
